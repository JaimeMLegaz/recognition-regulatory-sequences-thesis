...
seqs1 <- read_rds("../Data/table_GCB1_sequence.rds")
seqs2 <- read_rds("../Data/table_GCB2_sequence.rds")
...
```{r}
PosSequences <- read.csv("PositiveUTRSequences_untreated_sid", stringsAsFactors = FALSE)
NegSequences <- read.csv("NegativeUTRSequences_untreated_sid", stringsAsFactors = FALSE)
```
...
evalSequences$sequence <- padding_onehot(evalSequences$sequence)
...
```{r}
PosUTR <- scan(file="../Data/Pos3UTR_bgene",what="character")
length(PosUTR)
PosSequences_nodup <- PosSequences[-which(duplicated(PosUTR)),]
PosUTR <- PosUTR[-which(duplicated(PosUTR))]
length(PosUTR)

NegUTR <- scan(file="../Data/Neg3UTR_bgene",what="character")
length(NegUTR)
