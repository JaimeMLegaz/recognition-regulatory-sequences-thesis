...
```{r}
if (length(PosUTR) > length(NegUTR)){
  chosenSeqs <- sample(length(PosUTR), size = length(NegUTR), replace = FALSE, prob = NULL)
  PosUTR <- PosUTR[chosenSeqs]
}
if (length(NegUTR) > length(PosUTR)){
  chosenSeqs <- sample(length(NegUTR), size = length(PosUTR), replace = FALSE, prob = NULL)
  NegUTR <- NegUTR[chosenSeqs]
}
