...
```{r}
fileCon<-file("PosSeqBasic")
write.table(PosSequences,file = fileCon, quote=FALSE, row.names = FALSE, col.names = FALSE)
fileCon<-file("NegSeqBasic")
write.table(NegSequences,file = fileCon, quote=FALSE, row.names = FALSE, col.names = FALSE)
```
...
for (i in 1:30){
  relevances <- lrp_nonorm(modelConvolu, posS, i)[["input"]]
...
for (i in 1:30){
  relevances <- lrp_nonorm(modelConvolu, negS, i)[["input"]]
