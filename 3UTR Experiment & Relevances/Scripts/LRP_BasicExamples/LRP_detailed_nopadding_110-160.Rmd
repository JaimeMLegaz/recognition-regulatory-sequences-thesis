...
PosSequences$sequence <- padding_sequences(PosSequences$sequence, length=100) # Check the Study_Length_Padding markdown to know why 202 length
NegSequences$sequence <- padding_sequences(NegSequences$sequence, length=100)
...
# Validation
test1 = all(sapply(list(min(nchar(PosSequences$sequence)),max(nchar(PosSequences$sequence)), mean(nchar(PosSequences$sequence))), function(x) x == 100))
test2 = all(sapply(list(min(nchar(NegSequences$sequence)),max(nchar(NegSequences$sequence)), mean(nchar(NegSequences$sequence))), function(x) x == 100))
...
# Validation
test1 = all(sapply(list(min(nchar(PosSequences$sequence)),max(nchar(PosSequences$sequence)), mean(nchar(PosSequences$sequence))), function(x) x == 200))
test2 = all(sapply(list(min(nchar(NegSequences$sequence)),max(nchar(NegSequences$sequence)), mean(nchar(NegSequences$sequence))), function(x) x == 200))
if (test1 == FALSE) print ("ERROR 1")
if (test2 == FALSE) print ("ERROR 2")
```
...
for (i in 1:5){
  train.x <- append(train.x, list(data.matrix(trains[[i]][,201:400])))
  train.y <- append(train.y, list(data.matrix(trains[[i]][,401])))
  
  test.x <- append(test.x, list(data.matrix(tests[[i]][,201:400])))
  test.y <- append(test.y, list(data.matrix(tests[[i]][,401])))

  train.x[[i]] <- array_reshape(train.x[[i]], c(nrow(trains[[i]]),200,1))
  test.x[[i]] <- array_reshape(test.x[[i]], c(nrow(tests[[i]]),200,1))
 }
```
...
finalSequencesN <- data.matrix(dfn[,201:400])
finalSequencesN <- array_reshape(finalSequencesN,c(nrow(finalSequencesN),200,1))
finalSequencesP <- data.matrix(dfp[,201:400])
finalSequencesP <- array_reshape(finalSequencesP,c(nrow(finalSequencesP),200,1))
...
lrp <- function(model, input){ # Asks for the model and the input sequence
  ...
  for (i in 1:length(layers)){ # For each layer, we will save all the activations of its neurons, and weights of its connections
    ...
    intermediate_output <- predict(intermediate_layer_model, array_reshape(input, c(1,200,1))) # Gets the output of that layer for the selected sequence
  ...
  for (i in 1:200){ # Creating the data structures for the Relevance and the times used, and setting the values to zero
    Rk[["input"]][i] = 0
    Rk[["times-used"]][i] = 0
  }
...
for (i in 1:50){ # We create the empty list of relevances, that will later contain the relevances for the positions of the sequences
...
for (i in 1:10){ # For each sequence we want to calculate its relevance:
  relevances <- lrp(modelConvolu, as.matrix(as.numeric(posS[i,1:200]))) # Apply the LRP algorithm and obtain the relevance of each one-hot digit
