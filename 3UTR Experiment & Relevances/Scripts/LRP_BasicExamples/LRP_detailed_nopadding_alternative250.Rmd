...
```{r}
PosSequences <- PositiveSequences[nchar(PositiveSequences$sequence) >= 250,]
NegSequences <- NegativeSequences[nchar(NegativeSequences$sequence) >= 250,]
```
...
PosSequences$sequence <- padding_sequences(PosSequences$sequence, length=250) # Check the Study_Length_Padding markdown to know why 202 length
NegSequences$sequence <- padding_sequences(NegSequences$sequence, length=250)
...
# Validation
test1 = all(sapply(list(min(nchar(PosSequences$sequence)),max(nchar(PosSequences$sequence)), mean(nchar(PosSequences$sequence))), function(x) x == 1000))
test2 = all(sapply(list(min(nchar(NegSequences$sequence)),max(nchar(NegSequences$sequence)), mean(nchar(NegSequences$sequence))), function(x) x == 1000))
if (test1 == FALSE) print ("ERROR 1")
if (test2 == FALSE) print ("ERROR 2")
```
...
We want a balanced set, so we select the same number of positives and negatives.

```{r}
chosenSeqs <- sample(length(PosUTR), size = length(NegUTR), replace = FALSE, prob = NULL)
PosUTR <- PosUTR[chosenSeqs]
...
for (i in 1:5){
  train.x <- append(train.x, list(data.matrix(trains[[i]][,1:1000])))
  train.y <- append(train.y, list(data.matrix(trains[[i]][,1001])))
  
  test.x <- append(test.x, list(data.matrix(tests[[i]][,1:1000])))
  test.y <- append(test.y, list(data.matrix(tests[[i]][,1001])))

  train.x[[i]] <- array_reshape(train.x[[i]], c(nrow(trains[[i]]),1000,1))
  test.x[[i]] <- array_reshape(test.x[[i]], c(nrow(tests[[i]]),1000,1))
 }
```
...
genesP <- PosSequences_nodup[chosenSeqs,]
...
finalSequencesN <- data.matrix(dfn[,1:1000])
finalSequencesN <- array_reshape(finalSequencesN,c(nrow(finalSequencesN),1000,1))
finalSequencesP <- data.matrix(dfp[,1:1000])
finalSequencesP <- array_reshape(finalSequencesP,c(nrow(finalSequencesP),1000,1))
...
lrp <- function(model, input){ # Asks for the model and the input sequence
  ...
  for (i in 1:length(layers)){ # For each layer, we will save all the activations of its neurons, and weights of its connections
    ...
    intermediate_output <- predict(intermediate_layer_model, array_reshape(input, c(1,1000,1))) # Gets the output of that layer for the selected sequence
  ...
  for (i in 1:1000){ # Creating the data structures for the Relevance and the times used, and setting the values to zero
    Rk[["input"]][i] = 0
    Rk[["times-used"]][i] = 0
  }
...
```{r, include=FALSE}
# CHECKEANDO QUE ESTEMOS USANDO LAS CADENAS CORRECTAS EN DIRECCION CORRECTA
# Para ello, GenesN[1] debería coincidir con negS[1]

seqtest <- genesN$rawseq[1]
seqtest <- strReverse(seqtest)
seqtest <- padding_sequence(seqtest, len=250)
seqtest <- simple_onehot(seqtest)
...
for (i in 1:10){ # For each sequence we want to calculate its relevance:
  relevances <- lrp(modelConvolu, as.matrix(as.numeric(posS[i,1:1000]))) # Apply the LRP algorithm and obtain the relevance of each one-hot digit
  
  for (j in 1:50){ # Since we want the relevances of the characters (250), not the one-hot digits (1250), we have to combine them
...
for (i in 1:10){ # We do the same for the negative sequences
  relevances <- lrp(modelConvolu, as.matrix(as.numeric(negS[i,1:200])))
