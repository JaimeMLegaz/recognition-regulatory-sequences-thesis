...
for (i in 1:5){
  train.x <- append(train.x, list(data.matrix(trains[[i]][,201:808])))
  train.y <- append(train.y, list(data.matrix(trains[[i]][809])))
  
  test.x <- append(test.x, list(data.matrix(tests[[i]][,201:808])))
  test.y <- append(test.y, list(data.matrix(tests[[i]][,809])))

  train.x[[i]] <- array_reshape(train.x[[i]], c(nrow(trains[[i]]),608,1))
  test.x[[i]] <- array_reshape(test.x[[i]], c(nrow(tests[[i]]),608,1))
 }
```
...
finalSequencesN <- data.matrix(dfn[,201:808])
finalSequencesN <- array_reshape(finalSequencesN,c(nrow(finalSequencesN),608,1))
finalSequencesP <- data.matrix(dfp[,201:808])
finalSequencesP <- array_reshape(finalSequencesP,c(nrow(finalSequencesP),608,1))
...
```{r}
  ...
  modelConvolu %>% 
    layer_conv_1d(filters = 12, kernel_size = 20, activation = "relu", input_shape = input_shape)%>%
    layer_flatten() %>%
