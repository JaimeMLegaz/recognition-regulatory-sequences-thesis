...
PosSequences$sequence <- padding_sequences(PosSequences$sequence, length=50) # Check the Study_Length_Padding markdown to know why 202 length
NegSequences$sequence <- padding_sequences(NegSequences$sequence, length=50)
...
# Validation
test1 = all(sapply(list(min(nchar(PosSequences$sequence)),max(nchar(PosSequences$sequence)), mean(nchar(PosSequences$sequence))), function(x) x == 50))
test2 = all(sapply(list(min(nchar(NegSequences$sequence)),max(nchar(NegSequences$sequence)), mean(nchar(NegSequences$sequence))), function(x) x == 50))
...
for (i in 1:5){
  train.x <- append(train.x, list(data.matrix(trains[[i]][,1:200])))
  train.y <- append(train.y, list(data.matrix(trains[[i]][,201])))
  
  test.x <- append(test.x, list(data.matrix(tests[[i]][,1:200])))
  test.y <- append(test.y, list(data.matrix(tests[[i]][,201])))
...
finalSequencesN <- data.matrix(dfn[,1:200])
finalSequencesN <- array_reshape(finalSequencesN,c(nrow(finalSequencesN),200,1))
finalSequencesP <- data.matrix(dfp[,1:200])
