...
dfrows <- 1:(nrow(df))
fortrain <- dfrows[! dfrows %in% partitions[[1]]]

trained_pos <- fortrain[which(fortrain <= nrow(genesP))]
trained_neg <- fortrain[which(fortrain > nrow(genesP))] - nrow(genesP)
...
for (i in 1:nrow(negS)){
  ...
  for (j in 1:202){
    ...
    if (number > 1){
      errors <- c(errors, k)
      write.table(k, "../../Results/Relevances/negerrores")
    }
 ...
 # write.csv(relevancesN[1:i,],"../../Results/Relevances/negRelevances", row.names = FALSE)
  write.table(relevancesN[i,], "../../Results/Relevances/negRelevances", row.names = FALSE, col.names = (i == 1), append = (i != 1), sep = ",")
}
