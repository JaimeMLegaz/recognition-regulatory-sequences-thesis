...
# Indicate the sequences used for the training of model 1
relevancesP$trained1 <- 0
relevancesN$trained1 <- 0

trained_pos <- partitions[which(partitions[,1] <= nrow(genesP)),1]
trained_neg <- (partitions[which(partitions[,1] > nrow(genesP)),1] - nrow(genesP))

relevancesP[trained_pos,]$trained1 <- 1
relevancesN[trained_neg,]$trained1 <- 1
...
for (i in 1:nrow(posS)){
  ...
  relevances <- lrp(modelConvolu, as.matrix(as.numeric(posS[i,1:808])))
  ...
  for (j in 1:202){
    ...
    if (number > 1){
      errors <- c(errors, k)
      write.table(k, "../../Results/Relevances/poserrores")
    }
 ...
 # write.csv(relevancesP[i,],"../../Results/Relevances/posRelevances", row.names = FALSE, append=TRUE)
  write.table(relevancesP[i,], "../../Results/Relevances/posRelevances", row.names = FALSE, col.names = (i == 1), append = (i != 1), sep = ",")
}

## LECTURA CON read.table("../../Results/Relevances/posRelevances",  sep=",", header=TRUE, stringsAsFactors = FALSE,colClasses="character")