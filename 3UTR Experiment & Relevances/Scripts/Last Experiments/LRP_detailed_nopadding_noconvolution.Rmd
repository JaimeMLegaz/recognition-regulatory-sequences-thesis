...
```{r}
  ...
  modelConvolu %>% 
   layer_dense(units = 18, activation = 'relu', input_shape = input_shape) %>% 
    layer_dense(units = 1, activation="sigmoid") 
    
    summary(modelConvolu)
...
for (i in 1:5){
  finaltrainsx <- append(finaltrainsx, list(sapply(trains[[i]], as.numeric)[,1:808]))
finaltrainsy <- append(finaltrainsy, list(sapply(trains[[i]], as.numeric)[,809]))

finaltestx <- append(finaltestx, list(sapply(tests[[i]], as.numeric)[,1:808]))
finaltesty <- append(finaltesty, list(sapply(tests[[i]], as.numeric)[,809]))
}
...
for (i in 1:5){
  ...
  history <- NULL
   ...
   modelConvolu <- keras_model_sequential()
  modelConvolu %>% 
    layer_dense(units = 18, activation = 'relu', input_shape =808) %>% 
    layer_dense(units = 1, activation="sigmoid") 
  ...
  modelConvolu %>% compile(
    ...
    historial <- modelConvolu %>% fit(
      ...
      validation_data = list(finaltestx[[i]], finaltesty[[i]]),
      verbose = 2)
...
lrp <- function(model, input){ # Asks for the model and the input sequence
  ...
  for (i in 1:length(layers)){ # For each layer, we will save all the activations of its neurons, and weights of its connections
    ...
    intermediate_output <- predict(intermediate_layer_model, array_reshape(input, c(1,808))) # Gets the output of that layer for the selected sequence
  ...
  for (i in 1:length(weights[[2]]))
    pos_weights[i] <- max(weights[[2]][i],0) # Not using the apply family functions so it does not alter the shape
  
  denom = 0 # Denominator of the formula
  
  for (i in 1:length(activs[[1]])) { # Calculating the denominator
    value <- activs[[1]][i] * pos_weights[i]
    denom <- denom + value
  }
  
  for (i in 1:length(activs[[1]])) { # Calculating each numerator and, with it, the relevance of each neuron of the dense layer
    numer <- activs[[1]][i] * pos_weights[i]
  ...
  for (i in 1:length(input)) { # Calculating the denominator
    value <- input[i] * pos_weights[i]
    denom <- denom + value
  }
  
  for (i in 1:length(input)) { # Calculating each numerator and, with it, the relevance of each neuron of the dense layer
    numer <- input[i] * pos_weights[i]
    if (denom != 0) { # We do not want to divide by zero
        division <- numer/denom
      } else {
        division <- 0
      }
    Rk[["input"]][i] <- division * Rk[["dense"]][1] 
  }
...
for (i in 1:100){ # For each sequence we want to calculate its relevance:
...
for (i in 1:100){ # We do the same for the negative sequences
...
```{r}
plot(colSums(relevancesN[1:100,11:212])/100, type="l")

plot(colSums(relevancesP[1:100,11:212])/100, type="l")
```
