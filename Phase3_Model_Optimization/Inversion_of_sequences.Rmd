...
allgenes = read.delim("~/Dropbox/alumnos/jaimemartinez/tfg/workinggenes.txt",
                      stringsAsFactors=F,header=F)$V1
cat("We have",length(allgenes),"genes in our background set\n")
idgenes = read.delim("~/Dropbox/alumnos/jaimemartinez/tfg/vissersgenes.tsv",
                      stringsAsFactors=F,header=F)$V1
...
colnames(seqstosave) = c("gene","condition","sequence")
#write.table(seqstosave,"~/Dropbox/sequences/utrs/utrsSequences.tsv",sep="\t",row.names=F)
write.table(seqstosave,"~/Dropbox/sequences/utrs/utrsOnlyPositiveStrandSequences.tsv",sep="\t",row.names=F)
```
