...
```{r}
results = read.csv("../Results/finalEvaluationAllGenes",
                     stringsAsFactors=F)
```
...
But still, we want know to know the 5'UTR sequences that when used for training, were not classified correctly. Obviously, some genes/sequences will be used for training 5 times, others will be used 4 times and so on. So we have to separate across frequency of usage
...
write.table(final,"../Results/badseqs_CBAXXX.tsv",
...
sequences = results$raw5utr[results$diseaseAssociated == 1 & 
                                     results$model1 + results$model2 + results$model3 + 
                                     results$model4 + results$model5 == 5]
sequences
```
...
```{r}
write.table(sequences,"../Results/goodseqs_CBAXXX.tsv",
