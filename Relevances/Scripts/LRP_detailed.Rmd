...
For each conv_node in convolution_layer:
  ...
  For each input_node in inputs:
    ...
    times_used[input_node] += 1
    
return (Relevance[["input"]]/times_used)
    # We divide the relevances by the times each node was used to compensate the fact that the first and last nodes were used way less than the rest.
...
for (i in 1:5){ # One plot for every model
  ...
  plot(type="n", xlim <- c(1,250), c(min(pr_norm,nr_norm), max(pr_norm,nr_norm)), xlab = "Position of the character in the sequence", ylab="Relevance of the character", main=paste0("Relevances of every positive and negative sequence in model number ",i))
...
plot(type="n", xlim <- c(1,250), ylim <- c(min(as.data.frame(pos_rel)), max(as.data.frame(pos_rel))), xlab = "Position of the character in the sequence", ylab="Relevance of the character", main="Mean relevances of positive sequences in models")
...
plot(type="n", xlim <- c(1,250), ylim <-  c(min(as.data.frame(neg_rel)), max(as.data.frame(neg_rel))), xlab = "Position of the character in the sequence", ylab="Relevance of the character", main="Mean relevances of negative sequences in models")
...
- The dotted lines (the ones that we can see) tend to be in sequences with lower relevance
...
As we can see more clearly now, the pattern is very similar in both types of sequence, though the negative sequences tend to have lower relevances in general. This confirms one of our questions from before (both types having similar distributions). The question of wether negative sequences had lower relevances at the end of the sequence can be answered as well: They do have lower relevances in almost all the sequence.

We still have to check if dotted lines (padding sequences) tend to be in sequences with lower relevances. For that, we can divide each type of sequence in two subtypes: those sequences that contain one or more padding characters, and those that do not, and then plot them.
...
In this plot we can observe that sequences with no padding characters tend to have higher variation, while the sequences with padding have more consistent relevances. For the moment, we will remove from the plots those sequences that contain any padding character. Maybe later we will study this padding character again.

This would be the resultant plot without the sequences with padding:

```{r}

plot(type="n", xlim <- c(1,250), c(min(mean_nopad_pos_norm, mean_nopad_neg_norm), max(mean_nopad_pos_norm, mean_nopad_neg_norm)), xlab = "Position of the character in the sequence", ylab="Mean relevance of the character")
lines(mean_nopad_pos_norm, col="blue")
lines(mean_nopad_neg_norm, col="red")
legend("topright", legend=c("Positive no padding", "Negative no padding"),
       col=c("blue", "red"), lty=1, cex=0.8)
  

```

Without the padding sequences, positive and negative sequences still seem very similar. We can plot the difference between them to see more clearly their differences:

```{r}

mean_difference <- mean_nopad_pos_norm - mean_nopad_neg_norm
ggplot(as.data.frame(mean_difference), aes(x = 1:length(mean_difference), y = mean_difference)) + 
  geom_line(aes(col="Value of Positive - Negative"))+ 
...
Now we can easily see that the negative sequences have bigger relevances at the start and smaller at the end, while the middle remains territory of both. We can also check which if one of the two types has higher mean relevance:

```{r}

positive_relevance <- sum(mean_nopad_pos_norm)
negative_relevance <- sum(mean_nopad_neg_norm)
