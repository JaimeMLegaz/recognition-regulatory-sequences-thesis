...
With the exeption of the output layer, the relevance of each node j is equal to the sum of the relevances R(j<-k) for every node k in the last computed layer.

This R(j<-k) is equal to (Numerator(j,k)/Denominator(j,k))*R(k), being these:

- Numerator(j,k): The activation of the node k * the weight between the nodes j and k
...
num_seq <- nrow(pos_norm[[1]])

plot(type="n", xlim <- c(1,250), ylim <- c(0.6, 0.1), xlab = "Position of the character in the sequence", ylab="Relevance of the character", main="Mean relevances of positive sequences in models")
lines(colSums(pos_norm[[1]])/num_seq, col="blue")
lines(colSums(pos_norm[[2]])/num_seq, col="red")
lines(colSums(pos_norm[[3]])/num_seq, col="orange")
lines(colSums(pos_norm[[4]])/num_seq, col="black")
lines(colSums(pos_norm[[5]])/num_seq, col="purple")
...
num_seq <- nrow(neg_norm[[1]])

plot(type="n", xlim <- c(1,250), ylim <- c(0.8, 0), xlab = "Position of the character in the sequence", ylab="Relevance of the character", main="Mean relevances of negative sequences in models")
lines(colSums(neg_norm[[1]])/num_seq, col="blue")
lines(colSums(neg_norm[[2]])/num_seq, col="red")
lines(colSums(neg_norm[[3]])/num_seq, col="orange")
lines(colSums(neg_norm[[4]])/num_seq, col="black")
lines(colSums(neg_norm[[5]])/num_seq, col="purple")
...
And now we can do the same plot again, but this time with more sequences.
...
In this plot we can observe that, with some exceptions in certain small sections, sequences with no padding characters tend to have higher relevances. This could mean that our padding character is a handicap for the network trainning, and that an alternative could be desirable. 

We can be sure that the first character for some reason has a very high relevance. We could check this with more detail. We could also try to contrast this information with the relevances of other models just to be sure.
