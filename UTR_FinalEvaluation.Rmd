...
frameP <- data.frame(matrix(0, ncol=7, nrow=5))
colnames(frameP) <-  c("TrainedRight","TrainedWrong","UntrainedRight","UntrainedWrong", "RateTrainedRight", "RateUntrainedRight","DiseaseAssociated")

frameN <- data.frame(matrix(0, ncol=7, nrow=5))
colnames(frameN) <- c("TrainedRight","TrainedWrong","UntrainedRight","UntrainedWrong", "RateTrainedRight", "RateUntrainedRight","DiseaseAssociated")
```

```{r}
#saveVariable <- usedForTrainNeg
...
for (i in 1:5){
  ...
  countTrainNeg <- sum(genesN[trainedWithNeg,(4+i)] == 0) # Counts the number of genes used for training predicted as negative
  countUntrainNeg <- sum(genesN[notTrainedWithNeg,(4+i)] == 0) # Counts the number of genes not used for training predicted as negative
  frameN[i,] <- c(countTrainNeg,length(trainedWithNeg) - countTrainNeg, countUntrainNeg, length(notTrainedWithNeg) - countUntrainNeg, countTrainNeg/length(trainedWithNeg), countUntrainNeg/length(notTrainedWithNeg), 0)
  
  trainedWithPos <-usedForTrainPos[[i]]
  notTrainedWithPos <- allRowsP[-trainedWithPos]
  
  countTrainPos <- sum(genesP[trainedWithPos,(4+i)] == 1) # Counts the number of genes used for training predicted as positive
  countUntrainPos <- sum(genesP[notTrainedWithPos,(4+i)] == 1) # Counts the number of genes not used for training predicted as positive
  frameP[i,] <- c(countTrainPos,length(trainedWithPos) - countTrainPos, countUntrainPos, length(notTrainedWithPos) - countUntrainPos, countTrainPos/length(trainedWithPos), countUntrainPos/length(notTrainedWithPos), 1)
}
```

```{r}
# 10 to 14 
genesP$trainingModel1 <- 0
genesP$trainingModel2 <- 0
genesP$trainingModel3 <- 0
genesP$trainingModel4 <- 0
genesP$trainingModel5 <- 0

#15 to 19
genesP$evaluationModel1 <- 0
genesP$evaluationModel2 <- 0
genesP$evaluationModel3 <- 0
genesP$evaluationModel4 <- 0
genesP$evaluationModel5 <- 0

genesN$trainingModel1 <- 0
genesN$trainingModel2 <- 0
genesN$trainingModel3 <- 0
genesN$trainingModel4 <- 0
genesN$trainingModel5 <- 0

genesN$evaluationModel1 <- 0
genesN$evaluationModel2 <- 0
genesN$evaluationModel3 <- 0
genesN$evaluationModel4 <- 0
genesN$evaluationModel5 <- 0
