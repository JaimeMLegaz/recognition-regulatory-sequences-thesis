...
frameP <- data.frame(matrix(0, ncol=4, nrow=5))
colnames(frameP) <-  c("TrainedRight","TrainedWrong","UntrainedRight","UntrainedWrong")

frameN <- data.frame(matrix(0, ncol=4, nrow=5))
colnames(frameN) <- c("TrainedRight","TrainedWrong","UntrainedRight","UntrainedWrong")
```
...
for (i in 1:5){
  ...
  countTrainNeg <- sum(genesN[trainedWithNeg,(3+i)] == 0)
  countUntrainNeg <- sum(genesN[notTrainedWithNeg,(3+i)] == 0)
  frameN[i,] <- c(countTrainNeg,length(trainedWithNeg) - countTrainNeg, countUntrainNeg, length(notTrainedWithNeg) - countUntrainNeg)
  
  trainedWithPos <-usedForTrainPos[[i]]
  notTrainedWithPos <- allRowsP[-trainedWithPos]
  
  countTrainPos <- sum(genesP[trainedWithPos,(3+i)] == 1)
  countUntrainPos <- sum(genesP[notTrainedWithPos,(3+i)] == 1)
  frameP[i,] <- c(countTrainPos,length(trainedWithPos) - countTrainPos, countUntrainPos, length(notTrainedWithPos) - countUntrainPos)
}
```
...
```{r}
genesP$X5utr <- decode_onehot(genesP$X5utr)
genesN$X5utr <- decode_onehot(genesN$X5utr)
...
write.csv(frameP,file="Results/statsPositive",row.names = FALSE, col.names = FALSE, quote = FALSE)
write.csv(frameN,file="Results/statsNegative",row.names = FALSE, col.names = FALSE, quote = FALSE)
